...
fun calculate(packet: Packet): Long {
    return when (packet.type) {
        0 -> packet.children.sumOf { calculate(it) }
        1 -> packet.children.fold(1) { result, child -> result * calculate(child) }
        2 -> packet.children.minOf { calculate(it) }
        3 -> packet.children.maxOf { calculate(it) }
        4 -> packet.literal
        5 -> if (calculate(packet.children[0]) > calculate(packet.children[1])) 1L else 0L
        6 -> if (calculate(packet.children[0]) < calculate(packet.children[1])) 1L else 0L
        7 -> if (calculate(packet.children[0]) == calculate(packet.children[1])) 1L else 0L
        else -> 0
    }
}
